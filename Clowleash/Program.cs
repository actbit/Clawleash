...
using Clowleash.Plugins;
using Clowleash.Sandbox;
...
internal class Program
{
    ...
    {
        Console.WriteLine("================================");
        Console.WriteLine("   Clowleash Agent v1.0");
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(settings.AI.ApiKey))
            {
                ...
                Console.WriteLine("環境変数 CLowLEASH_API_KEY でも設定可能です");

                // 環境変数から取得を試行
                var envApiKey = Environment.GetEnvironmentVariable("CLOWLEASH_API_KEY");
    ...
    private static ClowleashSettings LoadSettings()
    {
        ...
        var settings = new ClowleashSettings();
        configuration.Bind(settings);

        return settings;
    }

    private static IServiceProvider ConfigureServices(ClowleashSettings settings)
    {
    ...
    private static Kernel BuildKernel(IServiceProvider serviceProvider, ClowleashSettings settings)
    {
    ...
    private static ChatCompletionAgent CreateAgent(IServiceProvider serviceProvider, ClowleashSettings settings)
    {
        ...
        var agent = new ChatCompletionAgent
        {
            Name = "ClowleashAgent",
            Instructions = """
                あなたはClowleash（クラウリッシュ）エージェントです。
