...
public class DockerSandboxProvider : ISandboxProvider
{
    ...
    public DockerSandboxProvider(ClowleashSettings settings)
    {
    ...
    private async Task<string?> CreateContainerAsync(CancellationToken cancellationToken)
    {
        ...
        var createArgs = $"create --name clowleash-sandbox-{Guid.NewGuid():N} {securityArgs} {mountArgs} {imageName} tail -f /dev/null";
